export class UpdateUserRequest {
  ...
  static create(object: { [key: string]: any }): [string?, UpdateUserRequest?] {
    const { firstName, lastName, email, password, newPassword, role, active, avatar } = object;
    ...
    if (active === undefined) return ['Status is required'];
    if (avatar && avatar.length === 0) return ['Avatar is required'];

    return [
      undefined,
      new UpdateUserRequest(firstName, lastName, email.toLowerCase(), password, role, active, avatar),
    ];
  }
}
